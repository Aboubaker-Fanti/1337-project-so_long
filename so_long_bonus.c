...
/*   Updated: 2023/02/19 11:56:48 by afanti           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (data->map[data->i])
	{
		...
		while (data->map[data->i][data->j])
		{
			if (data->map[data->i][data->j] == 'D')
				ft_images("./textures_bonus/5.xpm", data, data->i, data->j);
			else if (data->map[data->i][data->j] == 'E' )
				ft_images("./textures_bonus/or.xpm", data, data->i, data->j);
			else if (data->map[data->i][data->j] == '0')
				ft_images("./textures_bonus/2.xpm", data, data->i, data->j);
			else if (data->map[data->i][data->j] == 'P')
				ft_images("./textures_bonus/player.xpm", data, data->i,
					data->j);
			else if (data->map[data->i][data->j] == 'C')
				ft_images("./textures_bonus/colle1.xpm", data, data->i,
					data->j);
			else if (data->map[data->i][data->j] == 'N')
				ft_images("./textures_bonus/fire1.xpm", data, data->i, data->j);
			else if (data->map[data->i][data->j] == '1')
				ft_images("./textures_bonus/7.xpm", data, data->i, data->j);
			data->j++;
		}
		data->i++;
	}
...
{
	data->mapcopy = copy_of_mape(data->map);
	initialiser(data);
...
int	main(int ac, char **av)
{
	...
	if (ac == 2)
	{
		...
		if (!data->cc)
			return (0);
		check_av(av[1]);
		data->ptr = check_map_valide(av[1]);
		data->map = ft_split(data->ptr, '\n', ft_strlen(data->ptr));
		if (!data->map)
			exit(1);
		data->mapcopy1 = copy_of_mape(data->map);
		lastmain(data);
		firstmain(data);
		mlx_loop_hook(data->cc->mlx, fn_loop, data);
		mlx_hook(data->cc->mlx_win, 2, 0, render_next_frame, data);
		mlx_hook(data->cc->mlx_win, 17, 0, mousee, data);
